...
function save_export_to_server( $args = array() ) {
	...
	$file_path = WP_CONTENT_DIR . '/GatherPress-demo-data-2024.xml';
	file_put_contents( $file_path, $export_data );

	// Redirect to success page.
	wp_redirect( admin_url( 'export.php?save-export-to-server-success' ) );
	exit;
}
...
function save_export_to_server_admin_notice() {
	if ( ! isset( $_GET['save-export-to-server-success'] ) ) {
		return;
	}
	echo '<div class="notice notice-success"><p>Export saved successfully to the server!</p></div>';
}
